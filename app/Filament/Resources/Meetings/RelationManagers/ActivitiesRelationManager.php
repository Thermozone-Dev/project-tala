        ...
        return $table
            ...
            ->query(ActivityLogModel::query()->with(['causer', 'subject'])
                ->whereHasMorph('subject', [
                    ...
                    match ($type) {
                        Meeting::class => $query->where('id', $this->getOwnerRecord()->id),
                        default => $query->where('meeting_id', $this->getOwnerRecord()->id),
                    };
                })
                ->latest())
            ->columns([
                ...
                TextColumn::make('subject_name')
                    ...
                    ->getStateUsing(function (ActivityLogModel $record): ?string {
                        ...
                        $basename = class_basename($record->subject_type);
                        $subjectType = $labelMap[$basename] ?? Str::headline($basename);

                        $detail = match ($basename) {
                            'MeetingAttendee' => MeetingAttendee::find($record->subject_id)?->user?->full_name,
                            'Meeting' => Meeting::find($record->subject_id)?->title,        // adjust field name if different
                            'MeetingDocument' => MeetingDocument::find($record->subject_id)?->title, // adjust field name if different
                            default => null,
                        } ?? '—';

                        return "{$subjectType} : {$detail}";
                    })
                    ->placeholder('N/A'),
                ...
                Action::make('viewProperties')
                    ...
                    ->modalContent(function (ActivityLogModel $record): HtmlString {
                        ...
                        $valueMap = [
                            ...
                            'attendance_status_id' => fn($value) => \App\Models\MeetingAttendanceStatus::find($value)?->name ?? '—',
                            'mark_timestamp' => fn($value) => $value ? \Carbon\Carbon::parse($value)->format('M d, Y h:i A') : '—',
                            'updated_by' => fn($value) => \App\Models\User::find($value)?->name ?? '—',
                            'uploaded_by' => fn($value) => \App\Models\User::find($value)?->name ?? '—',
                        ];
